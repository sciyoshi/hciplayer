...
These tasks are analog to those asked of the user in the evaluation manual. There is no benchmark for the tutorial as it is a simple excercise focused on ensuring that the user has successfully performed every type of command in the system. While it is imperitive that it be completed it is not necessary that any results be derived from it.
\begin{enumerate}
\setcounter{enumi}{1} % Skip the first one (tutorial)
