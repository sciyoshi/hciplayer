...
To start this part of the evaluation, you need to have completed the tutorial, so as to be in the normal playback mode of the device. The examiners should read each of the commands, time the responses with a stopwatch, and record them on the \emph{benchmark tasks} sheet.
\begin{enumerate}
\setcounter{enumi}{9}
\item Play all songs by the artist \textit{Rage Against The Machine}
\item Play a song called \textit{Clocks}
\item Queue a song called \textit{Bulls on Parade}
